...
//	epiMask = ROIfolder + filenamePrefix + "-epiMask.tif";
	...
	if ( endsWith(f, "-epiMask.tif") ) {
		if ( File.exists(basalMask) && File.exists(apicalMask) ) { continue; } //skip if already processed
		open(inputFolder + f); rename("epi"); id0 = getImageID();
